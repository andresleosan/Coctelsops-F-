...
const blank = { code: "", startsAt: "", endsAt: "", discountType: "percent", discountValue: "", minimumSubtotal: "0", maxDiscount: "", active: true };
...
export default function PromotionsPage() {
  ...
  async function create(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/admin/promotions", { method: "POST", headers: { "content-type": "application/json", Authorization: `Bearer ${await user.getIdToken()}` }, body: JSON.stringify({ ...form, startsAt: `${form.startsAt}T00:00:00.000Z`, endsAt: `${form.endsAt}T23:59:59.999Z`, discountValue: Number(form.discountValue), minimumSubtotal: Number(form.minimumSubtotal), maxDiscount: form.maxDiscount ? Number(form.maxDiscount) : undefined }) });
