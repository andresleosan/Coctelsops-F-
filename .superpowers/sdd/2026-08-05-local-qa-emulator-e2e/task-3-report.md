...
- `src/firebase/emulators.ts`, `src/firebase/index.ts`: hosts públicos configurables y validación fail-closed.
...
- `npm test -- tests/lib/playwright-config.test.ts tests/lib/e2e-cleanup.test.ts tests/lib/e2e-local-state.test.ts tests/lib/e2e-local-runner.test.ts tests/lib/firebase-emulators.test.ts`: 42 tests passed en la corrida dirigida final.
- `npm run test:e2e:local`: exit code 0; Firebase Emulator inició con configuración temporal, lock exclusivo y puertos loopback libres aunque `127.0.0.1:8080` estaba ocupado por un proceso ajeno. Playwright ejecutó 3/3 escenarios, 3 passed, 0 skipped; cleanup terminó y `.tmp/e2e/local-state.json` quedó ausente. La salida no imprimió cuerpos HTTP ni tokens de Auth.
- `npm test`: 42 archivos passed, 1 skipped; 200 tests passed, 4 skipped.
...
- Se genera un `firebase.json` temporal bajo `.tmp/e2e` ignorado por Git. Sus rutas `firestore.rules` y `firestore.indexes.json` son absolutas y apuntan al repositorio; no se usa `firebase deploy`.
- Los hosts privados y públicos llegan a setup, Next y Playwright. El SDK cliente usa referencias estáticas `NEXT_PUBLIC_*` y validación loopback estricta.
...
- `shouldUseFirebaseEmulators()` ahora lanza un error loopback cuando la bandera pública vale exactamente `true` y falta o es inválido cualquiera de los dos hosts públicos. No devuelve `false` silenciosamente ni permite fallback a Firebase normal.
- `getClientEmulatorHosts()` lee únicamente `NEXT_PUBLIC_FIREBASE_FIRESTORE_EMULATOR_HOST` y `NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_HOST`; las variables privadas quedan reservadas para Firebase Admin.
- El runner adquiere `.tmp/e2e/local-emulator.lock` con creación exclusiva antes de seleccionar puertos y lo elimina en `finally`; una segunda instancia falla sin tocar el lock ajeno.
- En Windows se usa `cmd.exe /d /c` explícito con `shell: false` y un script `.cmd` temporal para `firebase emulators:exec`; se mantiene el escape de argumentos y no se usa la concatenación implícita de Node.
- La configuración y el script temporales viven bajo `.tmp/e2e` con rutas relativas sin espacios para evitar ambigüedades del parser de `cmd.exe`; ambos se eliminan en `finally`.
- Las pruebas dirigidas de emulators cubren hosts públicos ausentes, inválidos, remotos y frontera privada; las del runner cubren exclusión entre instancias y liberación del lock.
